...
def getPro():
    ...
    for i,rw in dfExc.iterrows():        
        if pd.isnull(dfExc.loc[i,'Service Code']):
            ...
            CData.to_excel('out.xlsx',index=False)
            # if len(CDataN)>0:
...
def fnlPro():
    ...
    for fl in fVyg:
        ...
        if len(df)>0:
            # DP|DD|DR
            vl = entry.get()
            df = df[(df['SIZE_TYPE'].astype(str).str.contains('HC|ST|TK')) & (~df['MOVE_TERMS'].astype(str).str.contains(vl))]    
...
button = ttk.Button(root, text='GetVsl_Schedule',command=getPro)
button.grid(row=5,column=1,padx = 5, pady = 40)

btn = ttk.Button(root, text='Process_Mails',command=fnlPro)
btn.grid(row=20,column=1 ,padx = 5, pady = 40)
